...
export default function TablaCompleta({ columns, data }) {
      ...
      <table className="min-w-full text-left text-sm font-light">
        ...
        <tbody>
          {table.getRowModel().rows.map((row, index) => (
            <tr
              key={index}
              className="border-b transition duration-300 ease-in-out text-white font-semibold hover:font-bold dark:border-neutral-500 "
            >
              {row.getVisibleCells().map((cell, index) => (
                <td
                  key={index}
                  className="whitespace-nowrap px-6 py-4  hover:text-indigo-800 hover:bg-neutral-100 dark:hover:bg-neutral-600"
                >
                  <Link
                    to={
                      index == 0
